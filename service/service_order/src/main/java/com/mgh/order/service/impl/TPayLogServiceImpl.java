...
import com.mgh.order.mapper.TPayLogMapper;
import com.mgh.order.service.TPayLogService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.mgh.order.utils.HttpClient;
...
public class TPayLogServiceImpl extends ServiceImpl<TPayLogMapper, TPayLog> implements TPayLogService {
    ...
    public Map createNative(String orderNo) {
        try {
            ...
            m.put("notify_url", "https://api.mch.weixin.qq.com/pay/unifiedorder\n");
    ...
    public Map<String, String> queryOrderStatus(String orderNo) {
        try {
            ...
            //3、返回第三方的数据
            String xml = client.getContent();
            Map<String, String> resultMap = WXPayUtil.xmlToMap(xml); //4、转成Map
            //5、返回
            return resultMap;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updateOrderStatus(Map<String, String> map) {
        //获取订单id
        String orderNo = map.get("out_trade_no"); //根据订单id查询订单信息
        QueryWrapper<TOrder> wrapper = new QueryWrapper<>();
        wrapper.eq("order_no", orderNo);
        TOrder order = orderMapper.selectOne(wrapper);
        if (order.getStatus().intValue() == 1) return;
        order.setStatus(1);
        orderMapper.updateById(order); //更新数据
        //记录支付日志
